...
pygame.display.set_caption("StreetRacer")#Name of the window
clock = pygame.time.Clock()#To set a FPS(Frame per seconds)
...
class Player(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.dx = 3
        self.dy = 3
    #Moving a blue car, with key events
    def update(self):
...
#code the movement of graphic objects
...
while restart:
    ...
    while running:
        ...
        for _ in coincoll:
         ...
         #To increase the speed of enemies that depends on the amount of coins that you got   
        if COINS % 3 == 0 and COINS > 0:
            COINS+=1
            E1.dy += 0.3
        #Screen to show that you lost end of the game
        while lose:
            #event_lose
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
    ...
    pygame.display.update()
pygame.quit()
#Complexity of this algorithm is approximately O(N^3)    